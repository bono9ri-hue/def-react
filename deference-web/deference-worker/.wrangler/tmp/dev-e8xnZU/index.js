...
// .wrangler/tmp/bundle-9omZ4J/checked-fetch.js
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path === "/assets" && request.method === "PUT") {
        ...
        if (body.memo !== void 0) await env.DB.prepare("UPDATE assets SET memo = ? WHERE id = ? AND user_id = ?").bind(body.memo, id, userId).run();
        if (body.tags !== void 0) await env.DB.prepare("UPDATE assets SET tags = ? WHERE id = ? AND user_id = ?").bind(body.tags, id, userId).run();
        if (body.folder !== void 0) await env.DB.prepare("UPDATE assets SET folder = ? WHERE id = ? AND user_id = ?").bind(body.folder, id, userId).run();
        if (body.palette_data !== void 0) await env.DB.prepare("UPDATE assets SET palette_data = ? WHERE id = ? AND user_id = ?").bind(body.palette_data, id, userId).run();
        if (body.status !== void 0) await env.DB.prepare("UPDATE assets SET status = ? WHERE id = ? AND user_id = ?").bind(body.status, id, userId).run();
        return new Response(JSON.stringify({ success: true }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
      ...
      if (path === "/collections") {
        ...
        if (request.method === "POST") {
          ...
          await env.DB.prepare("INSERT INTO collections (name, user_id, sort_order, created_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP)").bind(body.name || "", userId, nextOrder).run();
          return new Response(JSON.stringify({ success: true }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
        }
        if (request.method === "PUT") {
          ...
          if (Array.isArray(body)) {
            const statements = body.map(
              (col) => env.DB.prepare("UPDATE collections SET sort_order = ? WHERE id = ? AND user_id = ?").bind(col.sort_order, col.id, userId)
            );
...
// .wrangler/tmp/bundle-9omZ4J/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-9omZ4J/middleware-loader.entry.ts
