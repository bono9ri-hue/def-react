...
const GalleryCard = React.memo(({ asset, onClick, isSelected, onToggleSelect, isSelectionMode, viewMode }) => {
  ...
  // 로딩 상태(Skeleton)를 위한 방어 로직
  if (!asset) {
    const isMasonry = viewMode === 'masonry';
    return (
      <div className={`gallery-item break-inside-avoid mb-6 relative rounded-[10px] overflow-hidden bg-surface ${isMasonry ? 'h-[240px]' : 'aspect-square'}`}>
        <div className="absolute inset-0 bg-content/5 animate-pulse" />
      </div>
    );
  }
  ...
  // 🧪 UI 테스트용 Mock URL (필요 시 실제 asset.video_url 로 복구)
  const displayVideoUrl = hasVideo ? 'https://www.w3schools.com/html/mov_bbb.mp4' : ''; // TODO: 실제 연동 시 asset.video_url 로 복구
  ...
  return (
    ...
    >
      <div className={`relative ${isMasonry ? '' : viewMode === 'grid' ? 'aspect-square' : 'aspect-video'} rounded-[10px] overflow-hidden bg-surface transition-shadow duration-300
        ...
        {/* 🖼️ Base Image: 항상 브라우저의 기본값으로 보이게 설정 */}
        <img
          src={asset.image_url}
          alt={asset.memo}
          onLoad={() => setImageLoaded(true)}
          className={`${isMasonry ? 'w-full h-auto block' : 'absolute inset-0 w-full h-full object-cover'} transition-opacity duration-300 ${imageLoaded ? 'opacity-100' : 'opacity-0'}`}
          loading="lazy"
        />

        {/* ✨ Skeleton Overlay: 썸네일 기준 로딩 즉시 제거 */}
        <div className={`absolute inset-0 bg-content/5 animate-pulse pointer-events-none transition-opacity duration-300 ${imageLoaded ? 'opacity-0' : 'opacity-100'} z-[1]`} />

        {/* 🎥 Video Overlay: 썸네일 바로 위에 위치하며 호버 시에만 노출 */}
        {hasVideo && (
          <>
            <video
              ref={videoRef}
              ...
              preload="metadata"
            />
            <div className={`absolute top-3 left-3 z-[11] bg-black/60 text-white/90 text-[10px] font-bold px-2.5 py-1 rounded-lg tracking-widest flex items-center gap-1.5 transition-opacity duration-300 ${imageLoaded ? 'opacity-100' : 'opacity-0'} group-hover:opacity-0`}>
...
function Dashboard() {
  ...
  const [activeCollectionId, setActiveCollectionId] = useState(null); // ✨ 추가 고유 ID 기반 하이라이트
  ...
  }, [userId, getAssets, getBookmarks, getCollections, activeTab]);
  ...
  const handleSaveWorkspace = async () => {
    try {
      // 서버 사양(북마크 방식)에 맞춰 기존 객체 전체를 유지하며 sort_order만 업데이트하여 전송
      const orderPayload = collections.map((col, index) => ({
        ...col,
        sort_order: index
      }));
      ...
      setIsEditMode(false);
      showToast("워크스페이스 순서가 저장되었습니다.", "success");
    } catch (err) {
  ...
  const handleReorderPinned = (newPinned) => {
    setCollections([...newPinned, ...unpinnedCols]);
  };
  const handleReorderUnpinned = (newUnpinned) => {
    setCollections([...pinnedCols, ...newUnpinned]);
  };
    ...
    <div className="h-screen w-full flex bg-background text-content overflow-hidden font-medium">
      ...
      <aside className="w-[var(--sidebar-width)] bg-sidebar border-r border-border flex flex-col h-full z-30 shrink-0">
        <div className="p-6 pb-2">
          ...
          <nav className="flex flex-col gap-1">
            ...
            <NavBtn active={activeTab === 'gallery'} onClick={() => { setActiveTab('gallery'); setActiveCollection(null); setActiveCollectionId(null); }} icon={<Image size={16} strokeWidth={1.5} />} label="갤러리" isSubItem={true} />
          ...
          <div
            className="mt-8 mb-2 px-3 py-1 flex items-center justify-between cursor-default transition-all duration-200 rounded-lg"
          >
          ...
          <div className={`relative group/col-list max-h-[420px] overflow-y-auto scrollbar-hide pr-1 pb-10 transition-all duration-300 ${isEditMode ? 'border border-dashed border-content/10 rounded-2xl p-1 bg-content/[0.01]' : ''}`}>
                ...
                {pinnedCols
                  ...
                  .map((col) => (
                    <Reorder.Item
                      ...
                      className="relative rounded-full select-none outline-none"
                    >
                      <NavBtn
            ...
            <Reorder.Group
              ...
              className={`flex flex-col gap-1 ${pinnedCols.length > 0 ? 'mt-3' : ''}`}
            >
              {unpinnedCols
                .filter(col => col.name.toLowerCase().includes(colSearchQuery.toLowerCase()))
                .map((col) => (
        ...
        <div className="mt-auto border-t border-border p-4">
          <nav className="flex flex-col gap-1">
            ...
            <button
              ...
              className="w-fit mt-4 px-3 py-1.5 rounded-lg border border-border/50 text-[10px] uppercase tracking-tighter font-black text-contentMuted hover:border-content/30 hover:text-content transition-all opacity-40 hover:opacity-100"
            >
              Edit Workspace
            </button>
          </nav>
      ...
      <main className="flex-1 flex flex-col h-full bg-primary relative overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pt-20 select-none" onMouseDown={handleGridMouseDown}>
          ...
          <div className="px-12 max-w-[1400px] mx-auto animate-slide-up">
            ...
            {/* 4️⃣ Filter & Sort Bar (NEW Design) */}
            <ControlBar
              activeTab={activeTab}
                ...
                <div ref={gridContainerRef}>
                  {viewMode === 'masonry' && (
                    <div className="columns-2 md:columns-3 lg:columns-4 xl:columns-5 gap-6 [column-fill:_balance]">
                      {(loading ? Array(12).fill(null) : filteredAssets).map((asset, idx) => (
                        <GalleryCard
                          key={asset ? asset.id : `skeleton-${idx}`}
                          asset={asset}
                          onClick={() => {
                            if (!asset) return;
                            isSelectionMode || selectedIds.size > 0 ? toggleSelect(asset.id) : openImageModal(asset);
                          }}
                          isSelected={asset ? selectedIds.has(String(asset.id)) : false}
          ...
          <div className="bg-background border border-border w-full max-w-sm rounded-[32px] overflow-hidden shadow-2xl animate-slide-up">
            ...
            <div className="px-8 py-4 border-t border-border/30">
              <form onSubmit={async (e) => {
                e.preventDefault();
                if (isSubmitting || !newCollectionName.trim()) return;
          ...
          <div className="relative z-10 w-full max-w-6xl h-full flex flex-col md:flex-row gap-8 items-center justify-center animate-slide-up">
            ...
            <div className="flex-1 h-full flex items-center justify-center relative group/canvas">
              ...
              {selectedAsset.video_url ? (
                <video
                  src="https://www.w3schools.com/html/mov_bbb.mp4" /* TODO: 실제 연동 시 {selectedAsset.video_url} 로 복구 */
      ...
      {isEditMode && (
        <div className="fixed bottom-12 left-1/2 -translate-x-1/2 z-[1000] animate-fade-in-up">
          <div className="bg-[#1c1c1c]/80 backdrop-blur-2xl text-white px-6 py-3 rounded-full flex items-center gap-6 shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/5">
            <div className="flex flex-col">
              <span className="text-[10px] font-black uppercase tracking-widest opacity-40">Workspace Editor</span>
              <span className="text-[12px] font-bold tracking-tight">커스텀 순서 편집 중</span>
            </div>
            <div className="h-8 w-[1px] bg-white/10" />
            <div className="flex items-center gap-2">
              <button
                onClick={handleCancelWorkspace}
                className="px-4 py-2 rounded-full text-[12px] font-bold hover:bg-white/5 transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={handleSaveWorkspace}
                className="px-6 py-2 rounded-full bg-white text-black text-[12px] font-bold hover:bg-white/90 transition-all active:scale-95 shadow-lg"
              >
                Save Changes
              </button>
            </div>
...
function NavBtn({ icon, label, active, onClick, badge, isSubItem, onEdit, onDelete, onTogglePin, isPinned, isEditMode }) {
  ...
  const isDragging = false; // framer-motion handles this now via whileDrag

  return (
    <div
      className={`relative group/nav w-full flex items-center justify-between px-3 ${isSubItem ? 'py-1' : 'py-1.5'} rounded-full group ${active ? 'bg-hover text-content' : `text-content ${!isEditMode ? 'hover:bg-hover' : ''}`}`}
    >
      <div
        onClick={onClick}
        className={`flex-1 flex items-center gap-3 min-w-0 ${isEditMode ? 'cursor-grab active:cursor-grabbing select-none' : 'cursor-pointer'}`}
      >
        {isEditMode && (
          <span className="shrink-0 text-contentMuted opacity-50">
            <GripVertical size={14} strokeWidth={1.5} />
          </span>
        )}
        <span className="opacity-70 relative shrink-0">
          {icon}
          {isPinned && (
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        {(onEdit || onDelete || onTogglePin) && (
          <div
            ref={buttonRef}
            className="opacity-0 group-hover/nav:opacity-40 hover:!opacity-100 transition-opacity p-1 rounded-md hover:bg-content/10 cursor-pointer"
