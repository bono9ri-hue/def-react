...
import { containsAllOfType } from "../utils/containsAllOf.js";

//#region src/customizables/elementDescriptors.ts
const CLASS_PREFIX = "cl-";
const OBJ_KEY_DELIMITER = "__";
const containsAllElementsConfigKeys = containsAllOfType();
...
* descriptor map
*/
const APPEARANCE_KEYS = containsAllElementsConfigKeys([
	"button",
	"input",
	"checkbox",
	"radio",
...
*/
const toTargettableClassname = (key) => {
...
const toObjectKey = (key) => {
	return key.replace(/([-][a-z])/, (match) => match[1].toUpperCase());
};
const createElementDescriptor = (key) => {
	...
	return {
		...
		getObjectKeyWithState: (state) => objectKey + OBJ_KEY_DELIMITER + state,
		getObjectKeyWithId: (idObj) => objectKey + OBJ_KEY_DELIMITER + idObj.id,
		getObjectKeyWithIdAndState: (idObj, state) => objectKey + OBJ_KEY_DELIMITER + idObj.id + OBJ_KEY_DELIMITER + state,
		setId: (id) => id ? {
			id,
			__type: "id"
		} : void 0
...
const createDescriptorMap = (keys = APPEARANCE_KEYS) => {
	return fromEntries(keys.map((key) => [toObjectKey(key), createElementDescriptor(key)]));
};
const descriptors = createDescriptorMap();

//#endregion
export { CLASS_PREFIX, descriptors };
//# sourceMappingURL=elementDescriptors.js.map