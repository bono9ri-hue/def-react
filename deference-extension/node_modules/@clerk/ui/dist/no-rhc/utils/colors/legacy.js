import { resolveCSSVariable } from "../cssVariables.js";

//#region src/utils/colors/legacy.ts
const abbrRegex = /^#([a-f0-9]{3,4})$/i;
const hexRegex = /^#([a-f0-9]{6})([a-f0-9]{2})?$/i;
const rgbaRegex = /^rgba?\(\s*([+-]?\d+)(?=[\s,])\s*(?:,\s*)?([+-]?\d+)(?=[\s,])\s*(?:,\s*)?([+-]?\d+)\s*(?:[,|\/]\s*([+-]?[\d\.]+)(%?)\s*)?\)$/;
const perRegex = /^rgba?\(\s*([+-]?[\d\.]+)\%\s*,?\s*([+-]?[\d\.]+)\%\s*,?\s*([+-]?[\d\.]+)\%\s*(?:[,|\/]\s*([+-]?[\d\.]+)(%?)\s*)?\)$/;
const hslRegex = /^hsla?\(\s*([+-]?(?:\d{0,3}\.)?\d+)(?:deg)?\s*,?\s*([+-]?[\d\.]+)%\s*,?\s*([+-]?[\d\.]+)%\s*(?:[,|\/]\s*([+-]?(?=\.\d|\d)(?:0|[1-9]\d*)?(?:\.\d*)?(?:[eE][+-]?\d+)?)\s*)?\)$/;
const hwbRegex = /^hwb\(\s*([+-]?\d{0,3}(?:\.\d+)?)(?:deg)?\s*,\s*([+-]?[\d\.]+)%\s*,\s*([+-]?[\d\.]+)%\s*(?:,\s*([+-]?(?=\.\d|\d)(?:0|[1-9]\d*)?(?:\.\d*)?(?:[eE][+-]?\d+)?)\s*)?\)$/;
const keywords = {
	black: [
		0,
		0,
...
const parseRgb = (str) => {
	...
	} else if (str in keywords) return keywords[str];
	else return null;
...
const parseHwb = function(str) {
	if (!str) return null;
	const match = str.match(hwbRegex);
	return match ? transformHslOrHwb(match) : null;
};
const transformHslOrHwb = (match) => {
...
const hslaTupleToHslaColor = (hsla) => {
	return {
		h: hsla[0],
		s: hsla[1],
		l: hsla[2],
...
const rgbaTupleToHslaColor = (rgb) => {
	const r = rgb[0] / 255;
	const g = rgb[1] / 255;
	const b = rgb[2] / 255;
	const a = rgb[3] ?? 1;
...
const hwbTupleToHslaColor = (hwb) => {
	return rgbaTupleToHslaColor(hwbTupleToRgbTuple(hwb));
};
const hslaColorToHslaString = ({ h, s, l, a }) => {
	return `hsla(${h}, ${s}%, ${l}%, ${a ?? 1})`;
};
const parse = (str) => {
	const colorStr = resolveCSSVariable(str) || str;
	const prefix = colorStr.substr(0, 3).toLowerCase();
	let res;
	if (prefix === "hsl") res = {
		model: "hsl",
		value: parseHsl(colorStr)
	};
	else if (prefix === "hwb") res = {
		model: "hwb",
		value: parseHwb(colorStr)
	};
	else res = {
		model: "rgb",
		value: parseRgb(colorStr)
	};
	if (!res || !res.value) throw new Error(`Clerk: "${colorStr}" cannot be used as a color within 'variables'. You can pass one of:
...
const toHslaColor = (str) => {
	const { model, value } = parse(str);
	switch (model) {
		case "hsl": return hslaTupleToHslaColor(value);
		case "hwb": return hwbTupleToHslaColor(value);
		case "rgb": return rgbaTupleToHslaColor(value);
	}
};
const toHslaString = (hsla) => {
	return typeof hsla === "string" ? hslaColorToHslaString(toHslaColor(hsla)) : hslaColorToHslaString(hsla);
};
const changeHslaLightness = (color, num) => {
	return {
		...color,
		l: color.l + num
	};
};
const setHslaAlpha = (color, num) => {
	return {
		...color,
		a: num
	};
};
const changeHslaAlpha = (color, num) => {
	return {
		...color,
		a: color.a ? color.a - num : void 0
	};
};
const lighten = (color, percentage = 0) => {
	if (!color) return;
...
const makeTransparent = (color, percentage = 0) => {
...
const adjustForLightness = (color, lightness = 5) => {
	if (!color) return;
	const hsla = colors.toHslaColor(color);
	if (!hsla) return color;
	if (hsla.l === 100) hsla.l = 95;
	else hsla.l = Math.min(hsla.l + 2 * lightness, 100);
