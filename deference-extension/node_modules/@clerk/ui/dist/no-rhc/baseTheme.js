...
const BUTTON_SOLID_SHADOW = (color) => `${BORDER_SHADOW_LENGTH} ${color}, 0px 1px 1px 0px rgba(255, 255, 255, 0.07) inset, 0px 2px 3px 0px rgba(34, 42, 53, 0.20), 0px 1px 1px 0px rgba(0, 0, 0, 0.24)`;
const BUTTON_OUTLINE_SHADOW = (color) => `${BORDER_SHADOW_LENGTH} ${color}, 0px 2px 3px -1px rgba(0, 0, 0, 0.08), 0px 1px 0px 0px rgba(0, 0, 0, 0.02)`;
const cardContentStyles = (theme) => ({
	borderWidth: 0,
	boxShadow: `${theme.shadows.$cardContentShadow}, ${BORDER_SHADOW_LENGTH} ${theme.colors.$borderAlpha50}`
});
const inputShadowStyles = (theme, colors) => {
	const idleShadow = [`0px 0px 0px 1px ${colors.idle1}`, theme.shadows.$input.replace("{{color}}", colors.idle2)].toString();
	const hoverShadow = [`0px 0px 0px 1px ${colors.hover1}`, theme.shadows.$input.replace("{{color}}", colors.hover2)].toString();
	return {
		boxShadow: idleShadow,
		"&:hover": { boxShadow: hoverShadow },
		"&:focus-within,&[data-focus-within=\"true\"]": { boxShadow: [hoverShadow, theme.shadows.$focusRing.replace("{{color}}", colors.focus)].toString() }
	};
};
const checkboxShadowStyles = (theme, colors) => {
	const idleShadow = [`0px 0px 0px 1px ${colors.idle1}`, theme.shadows.$input.replace("{{color}}", colors.idle2)].toString();
	const hoverShadow = [`0px 0px 0px 1px ${colors.hover1}`, theme.shadows.$input.replace("{{color}}", colors.hover2)].toString();
	return {
		boxShadow: idleShadow,
...
const clerkTheme = { elements: ({ theme }) => {
	return {
		button: {
			"&[data-variant=\"solid\"]": {
				borderWidth: "0px",
				...
				"&[data-color=\"primary\"]": {
					...
					"&:focus": { boxShadow: [BUTTON_SOLID_SHADOW(theme.colors.$primary500), theme.shadows.$focusRing.replace("{{color}}", theme.colors.$colorRing)].toString() }
				},
				"&[data-color=\"danger\"]": {
					boxShadow: BUTTON_SOLID_SHADOW(theme.colors.$danger500),
					"&:focus": { boxShadow: [BUTTON_SOLID_SHADOW(theme.colors.$danger500), theme.shadows.$focusRing.replace("{{color}}", theme.colors.$dangerAlpha200)].toString() }
				}
			},
			"&[data-variant=\"outline\"]": {
				borderWidth: 0,
				boxShadow: BUTTON_OUTLINE_SHADOW(theme.colors.$borderAlpha100),
				"&:focus": { boxShadow: [BUTTON_OUTLINE_SHADOW(theme.colors.$borderAlpha100), theme.shadows.$focusRing.replace("{{color}}", theme.colors.$colorRing)].toString() }
			},
		...
		badge: {
			borderWidth: 0,
			margin: "1px",
			"&[data-color=\"primary\"]": { boxShadow: `${BORDER_SHADOW_LENGTH} ${theme.colors.$borderAlpha100}, ${theme.shadows.$badge}` },
			"&[data-color=\"danger\"]": { boxShadow: `${BORDER_SHADOW_LENGTH} ${theme.colors.$dangerAlpha300}, ${theme.shadows.$badge}` },
			"&[data-color=\"success\"]": { boxShadow: `${BORDER_SHADOW_LENGTH} ${theme.colors.$successAlpha300}, ${theme.shadows.$badge}` },
			"&[data-color=\"warning\"]": { boxShadow: `${BORDER_SHADOW_LENGTH} ${theme.colors.$warningAlpha300}, ${theme.shadows.$badge}` }
		},
		...
		phoneInputBox: { ...inputStyles(theme) },
		formInputGroup: { ...inputStyles(theme) },
		...
		scrollBox: { ...cardContentStyles(theme) },
		userButtonPopoverMain: { ...cardContentStyles(theme) },
		organizationSwitcherPopoverMain: { ...cardContentStyles(theme) },
		menuList: {
			borderWidth: 0,
...
const getBaseTheme = (theme = "clerk") => {
	switch (theme) {
		case "simple": return simpleTheme;
		case "clerk":
		default: return clerkTheme;
	}
};
const baseTheme = clerkTheme;

//#endregion
export { baseTheme, getBaseTheme };
